...
use crate::{
    error::ContractError,
    handler::{exec as ExecHandler, query as QueryHandler},
    migrations::{ConfigV100, SendTypeV100},
    state,
    state::{ADMIN, ALLOCATION_HOLDINGS, CONFIG},
};
...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Deposit {
            flush,
        } => ExecHandler::execute_deposit(deps, env, info, flush),
        ...
        ExecuteMsg::RemoveAllocationDetail {
            name,
        ...
        ExecuteMsg::AddToFlushWhitelist {
            address,
        } => ExecHandler::execute_add_flush_whitelist(deps, env, info, address),
        ExecuteMsg::RemoveFromFlushWhitelist {
            address,
...
mod tests {
    ...
    mod instantiate {
        ...
        use crate::{
            ...
            error::ContractError,
            handler::query::query_allocation,
            test_helpers::{
        ...
        fn basic() {
            ...
            assert_eq!(
                query_allocation(deps.as_ref(), ALLOCATION_1.into()).unwrap().unwrap(),
                AllocationHolding {
        ...
        fn dupe_holdings() {
            ...
            let info = mock_info(CREATOR, &[]);
            let env = mock_env();
            let err = instantiate(deps.as_mut(), env, info, instantiate_msg).err().unwrap();
            match err {
