...
pub fn set_height(env: &mut Env, height: u64) {
    ...
    if diff.is_positive() {
        plus_height(env, diff.unsigned_abs() as u64);
    } else {
        minus_height(env, diff.unsigned_abs() as u64);
    }
}
...
pub fn expect_already_exists_err(result: &Result<Response, ContractError>) {
    match result {
        Ok(_) => panic!("Must return error"),
...
pub fn expect_not_found_err(result: &Result<Response, ContractError>) {
    match result {
        Ok(_) => panic!("Must return error"),
...
pub fn expect_exceed_limit_err(result: &Result<Response, ContractError>) {
    match result {
        Ok(_) => panic!("Must return error"),
...
pub fn expect_invalid_zero_amount_err(result: &Result<Response, ContractError>) {
    match result {
        Ok(_) => panic!("Must return error"),
